...
describe('Kle', async () => {
  ...
  it('renders Y offsets', () => {
    const keys = [
      Key.build({ row: 0, col: 0, w: 1, h: 1, x: 0, y: 0.54, label: '~' }),
      Key.build({ row: 0, col: 1, w: 1, h: 1, x: 1, y: 0.36, label: '!' }),
      Key.build({ row: 0, col: 3, w: 1, h: 1, x: 2, y: 0.18, label: '@' }),
      Key.build({ row: 0, col: 4, w: 1, h: 1, x: 3, y: 0, label: '#' }),
      Key.build({ row: 0, col: 5, w: 1, h: 1, x: 4, y: 0.18, label: '$' }),
      Key.build({ row: 0, col: 6, w: 1, h: 1, x: 5, y: 0.36, label: '%' }),

      Key.build({ row: 1, col: 0, w: 1, h: 1, x: 0, y: 1.54, label: 'Tab' }),
      Key.build({ row: 1, col: 1, w: 1, h: 1, x: 1, y: 1.36, label: 'Q' }),
      Key.build({ row: 1, col: 3, w: 1, h: 1, x: 2, y: 1.18, label: 'W' }),
      Key.build({ row: 1, col: 4, w: 1, h: 1, x: 3, y: 1, label: 'E' }),
      Key.build({ row: 1, col: 5, w: 1, h: 1, x: 4, y: 1.18, label: 'R' }),
      Key.build({ row: 1, col: 6, w: 1, h: 1, x: 5, y: 1.36, label: 'T' }),
    ];
    const kle = new Kle(keys);
    const json = kle.asJson();
    // console.log('json', kle.toKle());
    expect(json[0][0]).to.have.property('y', 0.54);
    expect(json[0][2]).to.have.property('y', -0.18);
    expect(json[1][0]).to.have.property('y', 0.18);
  });
});
